...
vigSrc = list.files(pattern = "Rnw$",
                    system.file("doc", package = "parallel"),
                    full.names = TRUE)
vigSrc
for (v in vigSrc) Stangle(v)

### R code from vignette source '/usr/local/lib64/R/library/parallel/doc/parallel.Rnw'
...
cd4.boot <- do.call(c, parallel::mclapply(seq_len(mc), run1) )
boot.ci(cd4.boot,  type = c("norm", "basic", "perc"),
        conf = 0.9, h = atanh, hinv = tanh)
})
...
cd4.boot <- do.call(c, parallel::parLapply(cl, seq_len(mc), run1) )
boot.ci(cd4.boot,  type = c("norm", "basic", "perc"),
        conf = 0.9, h = atanh, hinv = tanh)
...
###################################################
#Running the double bootstrap on the same problem is far more computer-intensive.
...
cd4.nest <- do.call(c, parallel::mclapply(seq_len(mc), run2, mc.cores = mc) )
nominal <- (1:R)/(R+1)
actual <- cd4.nest$t[, 2]
100*nominal[c(sum(actual <= 0.05), sum(actual < 0.95))]
})
