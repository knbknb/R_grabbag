...
to set-label
  ifelse v > -1 [set plabel v] [set plabel ""]
end
...
to accelerate
  ...
  let n 0
  let car-here? false
  while [d <= vmax + 1 and car-here? = false]
    [
    ifelse pxcor + d > world-width - 1 [set n pxcor + d - world-width] [set n pxcor + d]
    ask patch n 0 [ifelse v >= 0 [set car-here? true] [set d d + 1] ]
    ]
  ifelse d > v + 1 and v < vmax
    [
    set v v + 1
    ]
    [
...
to randomization
  if v > 0 and random-float 1 < p [set v v - 1]
end
