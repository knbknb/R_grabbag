...
if(exists("old_users") & table.type == "users"){
        ...
        stop("no users table found, no followers table found, no old tweets found")
}
dbDisconnect(conn)
...
#nusrs <- length(users)
nusrs <- 10
for(k in seq(x, nusrs, by=batchsize_userget )){
#for(k in seq(1, nusrs, by=batchsize_userget )){
#for(k in sample(1:length(users), nusrs) ){
        i <- k 
        ...
        if(i <= length(users)){
                tryCatch({
                        ...
                        if (table.type == "followers" & as.integer(rlim[rlim$resource == "/followers/ids", "remaining"]) > 0){
                                ...
                                followees <- lookupUsers(users[i:min(i+batchsize_userget, length(users))])
                                ...
                                tusers <- lapply(as.list(followees), function(user) {
                                        list(followers = getFollowersList(user, nMax = 2, retryOnRateLimit=10000))
                                ...
                                res <- lapply(tusers, function(followee){
                                        ...
                                        screen.name <- followee[["user"]]
                                        #screen.name <- followee[["user"]]
                                        printf(paste0(screen.name, "\n"))
                                        printf(paste0("\nlooked up followers for: ",screen.name, "\n"))
                                        if(is.character(screen.name)){
                                                printf(paste0("\n"))
                                                lapply(followee[["followers"]], function(follower){
                                                        printf(paste0("storing ", follower$getScreenName() , "\n"))
                                                        twitteR::store_users_db(follower, username.table)
...
if(table.type == "followers"){
        ...
        }, error=function(e){warning(e); return(paste("cannot get create table: ", table.users_followers))}
        )
...
if(table.type == "users"){
        ...
        tryCatch({
                        ...
                        twitteR::store_users_db(tusers, username.table)
                        userinfo[, "row_names"] <- NULL
                        append2SQLite(dfr = unique(userinfo), table.name = username.table, db.name = db.name)
                }, error=function(e){warning(e); return(paste("cannot get create table: ", username.table))
        })
}
